...
public final class TLCodecs {
    private static final Logger LOG = LoggerFactory.getLogger(TLCodecs.class);
    ...
    private static final Codec.ResultFunction<ItemStack> MISSING_CONTENT_ITEMSTACK_RESULT = new Codec.ResultFunction<>() {
        ...
        public <T> DataResult<T> coApply(DynamicOps<T> ops, ItemStack input, DataResult<T> t) {
            ...
            if (TLItems.MISSING_CONTENT.is(input)) {
                var originalData = input.get(TLComponents.MISSING_CONTENT_ITEMSTACK_DATA);
