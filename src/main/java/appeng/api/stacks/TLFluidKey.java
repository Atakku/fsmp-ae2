...
public final class TLFluidKey extends TLKey {
    public static final MapCodec<TLFluidKey> MAP_CODEC = RecordCodecBuilder.mapCodec(
            instance -> instance.group(
                    ...
                    .apply(instance, (fluidHolder,
                            dataComponentPatch) -> new TLFluidKey(new FluidStack(fluidHolder, 1, dataComponentPatch))));
    public static final Codec<TLFluidKey> CODEC = MAP_CODEC.codec();
    ...
    private TLFluidKey(FluidStack stack) {
        Preconditions.checkArgument(!stack.isEmpty(), "stack was empty");
        this.stack = stack;
        this.hashCode = FluidStack.hashFluidAndComponents(stack);
    }

    public static TLFluidKey of(Fluid fluid) {
        return of(new FluidStack(fluid, 1));
    }

    @Nullable
    public static TLFluidKey of(FluidStack fluidVariant) {
        if (fluidVariant.isEmpty()) {
            return null;
        }
        return new TLFluidKey(fluidVariant.copyWithAmount(1));
    }

    public static boolean matches(TLKey what, FluidStack fluid) {
        return what instanceof TLFluidKey fluidKey && fluidKey.matches(fluid);
    }

    public static boolean is(TLKey what) {
        return what instanceof TLFluidKey;
    }

    public static TLKeyFilter filter() {
        return TLFluidKey::is;
    }
    ...
    @Override
    public TLKeyType getType() {
        return TLKeyType.fluids();
    }

    @Override
    public TLFluidKey dropSecondary() {
    ...
    public boolean equals(Object o) {
        ...
        if (o == null || getClass() != o.getClass())
            return false;
        TLFluidKey aeFluidKey = (TLFluidKey) o;
    ...
    public static TLFluidKey fromTag(HolderLookup.Provider registries, CompoundTag tag) {
        var ops = registries.createSerializationContext(NbtOps.INSTANCE);
        try {
            return CODEC.decode(ops, tag).getOrThrow().getFirst();
        } catch (Exception e) {
            TLLog.debug("Tried to load an invalid fluid key from NBT: %s", tag, e);
            return null;
        }
    }
    ...
    public static TLFluidKey fromPacket(RegistryFriendlyByteBuf data) {
        var stack = FluidStack.STREAM_CODEC.decode(data);
        return new TLFluidKey(stack);
    }

    public static boolean is(@Nullable GenericStack stack) {
        return stack != null && stack.what() instanceof TLFluidKey;
    }
