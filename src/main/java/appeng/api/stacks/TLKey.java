 ...
 * </ul>
 */
public abstract class TLKey {
    public static final String TYPE_FIELD = "#t";
    private static final Logger LOG = LoggerFactory.getLogger(TLKey.class);
    public static final MapCodec<TLKey> MAP_CODEC = TLKeyType.CODEC
            .<TLKey>dispatchMap(TYPE_FIELD, TLKey::getType, TLKeyType::codec)
            .mapResult(new MapCodec.ResultFunction<>() {
                @Override
                public <T> DataResult<TLKey> apply(DynamicOps<T> ops, MapLike<T> input, DataResult<TLKey> a) {
                    if (a instanceof DataResult.Error<TLKey> error) {
                        ...
                        if (convert instanceof CompoundTag compoundTag) {
                            missingContent.set(TLComponents.MISSING_CONTENT_TLKEY_DATA, CustomData.of(compoundTag));
                        }
                        LOG.error("Failed to deserialize TL key: {}", error.message());
                        missingContent.set(TLComponents.MISSING_CONTENT_ERROR, error.message());

                        return DataResult.success(
                                TLItemKey.of(missingContent),
                                Lifecycle.stable());
                    }
                ...
                @Override
                public <T> RecordBuilder<T> coApply(DynamicOps<T> ops, TLKey input, RecordBuilder<T> t) {
                    // When the input is a MISSING_CONTENT item and has the original data attached,
                    // we write that back.
                    if (TLItems.MISSING_CONTENT.is(input)) {
                        var originalData = input.get(TLComponents.MISSING_CONTENT_TLKEY_DATA);
    ...
    public static final Codec<TLKey> CODEC = MAP_CODEC.codec();

    public static final StreamCodec<RegistryFriendlyByteBuf, TLKey> STREAM_CODEC = StreamCodec.of(
            TLKey::writeKey,
            TLKey::readKey);

    public static final StreamCodec<RegistryFriendlyByteBuf, TLKey> OPTIONAL_STREAM_CODEC = StreamCodec.of(
            TLKey::writeOptionalKey,
            TLKey::readOptionalKey);

    /**
     * Writes a generic, nullable key to the given buffer.
     */
    public static void writeOptionalKey(RegistryFriendlyByteBuf buffer, @Nullable TLKey key) {
    ...
    public static void writeKey(RegistryFriendlyByteBuf buffer, TLKey key) {
     ...
     */
    @Nullable
    public static TLKey readOptionalKey(RegistryFriendlyByteBuf buffer) {
    ...
    @Nullable
    public static TLKey readKey(RegistryFriendlyByteBuf buffer) {
        var id = buffer.readVarInt();
        var type = TLKeyType.fromRawId(id);
        if (type == null) {
            TLLog.error("Received unknown key space id %d", id);
            return null;
        }
    ...
    @Nullable
    public static TLKey fromTagGeneric(HolderLookup.Provider registries, CompoundTag tag) {
     ...
     * show it as 1 bucket, hence this method would return 1000 on Forge and 81000 on Fabric for TLFluidKey).
     */
    ...
    /**
     * @see TLKeyType#getAmountPerOperation()
     */
    ...
    /**
     * @see TLKeyType#getAmountPerByte()
     */
    ...
    /**
     * @see TLKeyType#formatAmount(long, AmountFormat)
     */
    ...
    /**
     * Tests if this and the given TL key are in the same fuzzy partition given a specific fuzzy matching mode.
     */
    public final boolean fuzzyEquals(TLKey other, FuzzyMode fuzzyMode) {
    ...
    /**
     * True to indicate that this type of {@link TLKey} supports range-based fuzzy search using
     * {@link TLKey#getFuzzySearchValue()} and {@link TLKey#getFuzzySearchMaxValue()}.
     * <p/>
