...
final class TooltipsSearchPredicate implements Predicate<GridInventoryEntry> {
    ...
    private final Map<TLKey, String> tooltipCache;

    public TooltipsSearchPredicate(String tooltip, Map<TLKey, String> tooltipCache) {
     ...
     */
    private String getTooltipText(TLKey what) {
        return tooltipCache.computeIfAbsent(what, key -> {
            var lines = TLKeyRendering.getTooltip(key);
            ...
            for (int i = 0; i < lines.size(); i++) {
                ...
                if (i > 0 && i >= lines.size() - 1 && !TLConfig.instance().isSearchModNameInTooltips()) {
